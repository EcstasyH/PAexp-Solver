...
%\newcommand{\znj}[1]{\todo[inline,color=orange!10]{\textbf{ZHAN:} #1}}
...
\begin{abstract}
    The problem of solving string constraints is currently one of the focuses of theoretical computer science, particular related to SMT-based program verification, as it is inevitably that most of 
    programs manipulate string data structures. 
    The general problem of string constraint solving 
    is undecidable, therefore people try to identify 
    some sub-problems with decidability, e.g., string constraints restricted to generic flat languages. 
    Certainly, it is challenging and 
    makes theoretical and practical significance to 
    identify such subset with as much expressive power as possible. 
    In this paper, we identify 
    the subset of flat string constraints with 
     the string-number conversion function $\mathbf{parseInt}$, and 
     provide a decision procedure to the subset by reduction to
   the decidability problem of Presburger Arithmetic with exponential functions, 
    which is decidable. To the best of our knowledge, such a subset is 
    the largest subset of string constraints with decidability so far. 

\keywords{ String Constraints \and Decidability \and Exponential Functions \and Quantifier Elimination.}
