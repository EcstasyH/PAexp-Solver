...
It is easy to observe that $c - |b_i|10^u \le c - b_i \ivarx_{\sigma(i)} \le c + |b_i|10^u$.
...
\sum_{j=1}^{i-1} a_j 10^{\ivarx_{\sigma(j)}} + \sum_{k=1}^{i-1}b_k \ivarx_{\sigma(k)} \le c - |b_i|10^u
\end{equation}
and
\begin{equation}\label{eqn-ub}
\sum_{j=1}^{i-1} a_j 10^{\ivarx_{\sigma(j)}} + \sum_{k=1}^{i-1}b_k \ivarx_{\sigma(k)} \le c + |b_i|10^u.
\end{equation}
Let $\alpha_1 = \ell_{10}(c - |b_i|10^u) -  \ell_{10}(a_{i-1})$ and $\alpha_2 = \ell_{10}(c + |b_i|10^u) -  \ell_{10}(a_{i-1})$. Then from Lemma~\ref{lem:exp-ineq}, 
\begin{itemize}
