...
During each iteration of the inner \emph{for} loop, if the maximal $x_i$ in $\bar{x}$ occurs in an exponential term in an atomic formula $\tau$, we will invoke \textbf{Elim-exp} to produce a new formula where $x_i$ occurs linearly equivalent to $\tau$. Then \textbf{Elim-linear} will eliminate all linear occurrences of $x_i$, which is similar to the classic Cooper's QE algorithm for PA \cite{Cooper72}. 
...
\subsection{Enumerate-Orders} 
In the \textbf{Normalization} step, we simplify the origin formula at the cost of more quantified variables. 
...
To the end, we denote the set of all orders among the $n+1$ variables by 
 ...
 order, to eliminate $x_{\sigma(i)}$ for $i=n$ to $0$ by invoking \textbf{Elim-exp} first, and then invoking \textbf{Elim-linear} (the inner \textit{for} loop). 
 In each iteration of the inner \textit{for} loop, 
if $x_{\sigma{(i)}}$ occurs in an exponential term in an atomic formula, 
\textbf{Elim-exp} is invoked and a formula   equivalent to the atom is returned, in which $x_i$ occurs linearly;  
then $x_i$ occurs linearly in the formula, thus \textbf{Elim-linear} is further invoked to eliminate all occurrences of $x_i$. 
...
\iffalse
    \For{i from $0$ to $n$}
    {
        ...
        \While{$x_i$ occurs in an exponential term in formula $\tau(x_i,\bar{x},\bar{y})$}{
            \tcp{ Elim-exp outputs a formula equivalent with $\tau(x_i,\bar{x},\bar{y})$ where $x_i$ occurs linearly }
            $\theta_i :=  \theta_i[\text{Elim-exp}(\tau(x_i,\bar{x},\bar{y}))/\tau(x_i,\bar{x},\bar{y})]$\;
        }   
        transform $\theta_i$ into disjunction normal form $\theta_i = \bigvee_s \theta_{i,s}$\;
        \For{ every $\theta_{i,s}$ }{
            $\theta'_{i,s} :=  \text{Elim-linear}(x_i,\theta_{i,s})$
        }
...
\textbf{Elim-exp} and \textbf{Elim-linear} are the most technical parts of our QE algorithm.
As mentioned, 
\textbf{Elim-exp} takes $\theta_{\sigma}(\bar{x},\bar{y})$ and  variable $x_{\sigma(i)}$ (the maximal variable among all quantified variables that are not eliminated yet according to $\sigma$) as 
...
We first give the whole procedure of \textbf{Elim-exp},
...
\begin{algorithm}[t]
    ...
    \While{there is an atom $\tau(x_i,\bar{x},\bar{y})$ contains $2^{x_i}$}
    {
        ...
        {
            \tcp{ \textbf{Elim-exp-div} case, the atom is of the form $d|t(x_i,\bar{x},\bar{y})$}
...
because \textbf{Enumerate-Orders} can not always be omitted and formulae in \textbf{Elim-exp} can not be simplified easily.
\fi