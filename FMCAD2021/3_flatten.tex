...
\vspace{-1mm}
 ...
 \end{array}
 $$
 The semantics of {\paexp} are defined similarly to {\pa} with the difference that variables are interpreted over $\Nat$, that is, an interpretation is a function $I: \free(\phi) \rightarrow \Nat$. The quantifier-free as well as existential {\paexp} formulas are defined similarly to {\pa} as well.
...
We will use the notation $\lambda_{10}(\iterm)$ to denote $10^{\ell_{10}(\iterm)}$. From the definition of $\lambda_{10}(\iterm)$, we know that for all $n \ge 1$, $\lambda_{10}(n) \le n < 10\lambda_{10}(n)$ holds.
