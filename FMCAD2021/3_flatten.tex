...
\begin{itemize}
\item The (partial) function $\ell_{10}(\ivarx)$ is inductively defined as follows: For $n \ge 1$, $\ell_{10}(n) = m$ iff $10^m \le n < 10^{m+1}$. (Note that $\ell_{10}(0)$ is undefined.) 
%
...
%
 $$
 \begin{array}{ l c l}
 \iterm &::=& c \mid \ivarx \mid \iterm + \iterm \mid \iterm - \iterm \mid 10^\iterm \mid \ell_{10}(\iterm) \mid \lambda_{10}(\iterm), \\
 \phi &::= & \iterm \ \op\ 0 \mid c | \iterm \mid \phi \wedge \phi \mid \phi \vee \phi \mid \neg \phi \mid \exists \ivarx.\ \phi \mid \forall \ivarx.\ \phi.
 \end{array}
 $$
...
\begin{example}
Suppose $\parseInt(\svarx) = 2\ivarx$ is an atomic constraint and $\flatfun_\varphi(\svarx) = (12)^*(34)^*$. Then 
\[
\small
\begin{array}{l l}
& \flatten_{\flatfun_\varphi}(\parseInt(\svarx)  =  2\ivarx)  \\
\equiv & \parseInt(12) \frac{10^{2\#_{\svarx,1}}-1}{10^2-1}10^{2\#_{\svarx,2}}  + \parseInt(34) \frac{10^{2\#_{\svarx,2}}-1}{10^2-1} = 2\ivarx   \\
\equiv & 12 (10^{2\#_{\svarx,1}+2\#_{\svarx,2}} - 10^{2\#_{\svarx,2}})  + 34 (10^{2\#_{\svarx,2}}-1) = 198\ivarx.
\end{array}
\]
