...
{\paexp} extends Presburger arithmetic with the exponential function $10^x$ as well as the (partial) functions $\ell_{10}(\ivarx)$ (cf. \cite{Point86}). The function $\ell_{10}(\ivarx)$ is inductively defined as follows. For $n \ge 1$, $\ell_{10}(n) = m$ iff $10^m \le n < 10^{m+1}$. (Note that $\ell_{10}(0)$ is undefined.)
...
\vspace{-1mm}
 ...
 \end{array}
 $$
 The semantics of {\paexp} are defined similarly to {\pa} with the difference that variables are interpreted over $\Nat$ (to avoid terms like $10^{-2}$), that is, an interpretation is a function $I: \free(\phi) \rightarrow \Nat$. 
...
\begin{theorem} \label{thm:string-parInt}
    \wuhao{The satisfiability of  $\strint$ under flat domain restrictions is equivalent to the satisfiability of existential $\paexp$ formulas, and thus is decidable. }
\end{theorem}
