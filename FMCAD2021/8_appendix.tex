...
Note that $\ell_{10}(n)$ is undefined for $n\le 0$ in $\paexp$. For convenience, we define $\ell_{10}(n)=0$ for $n\le 0$ in the following proof. 
...
\begin{proof}
By an induction on $n'$, we first show that for any $n' \in \Nat$, if $p \ge 2n'$, then $10^{n'} p \le 10^p$. 
