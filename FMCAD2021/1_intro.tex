...
The basic idea of our approach can be sketched as follows: we first reduce the satisfiability problem to the corresponding satisfiability problem of the theory of Presburger Arithmetic with exponential functions (denoted by $\paexp$), \wuhao{more precisely, the existential fragment of $\paexp$}; then, according to the decidability of $\paexp$, we obtain the decidability of the original satisfiability problem.  
...
\wuhao{We also analyze the deterministic upper bound of complexity for the first time: given a formula with only existential quantifiers, the complexity is 3-EXPSPACE to eliminate all quantifiers}. Furthermore, we propose various optimizations (Section~\ref{sec-opt}) and achieve the first prototypical implementation of Point's procedure (Section~\ref{sec-exp}). 
