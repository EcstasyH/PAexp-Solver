...
\paragraph*{Structure}
After an introduction to background knowledge in Section~\ref{sec:pre}, we describe how to flatten a string constraint with string-integer conversion to an $\paexp$ formula in Section~\ref{sec:string-solving}. A decision procedure of $\paexp$ formulae is presented in Section~\ref{sec-dec}. We present optimizations in Section~\ref{sec-opt} and show the empirical evaluation results in Section~\ref{sec-exp}.
