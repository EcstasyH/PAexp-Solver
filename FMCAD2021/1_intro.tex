...
Even more crucially, in many programming languages, the string-number conversion is a part of the definition of their core semantics \zhilin{reference here ?}. 
...
The idea is to restrict the solution space of string variables to some pattern defined by \emph{flat languages} (see Section~\ref{sec:string-solving} for more details). 
...
Second, and more importantly, because we can convert the membership problem of a flat language to the satisfiability problem of a Presburgher formula, the class of word equation + flat languages + length constraints is decidable.
...
The decidability of $\paexp$ was first shown by Semen\"{o}v in \cite{Semenov84}. Nevertheless, Semen\"{o}v did not provide an explicit decision procedure. To remedy this, in \cite{Point86}, Point presented the first quantifier elimination procedure for the satisfiability of $\paexp$. Partially attributed to the fact that Semen\"{o}v decidability proof in \cite{Semenov84} was quite abstract and Point's procedure in \cite{Point86} was presented  in a mathematical and dense way, this line of work has eluded the attentions of computer science researchers, to the best of our knowledge. Therefore, no implementation based on Point's procedure has been available up to now. 

Aiming at introducing this line of work to computer science researchers, we reformulate (and slightly improve) Point's procedure in a way, which, hopefully, is more accessible for computer science researchers (Section~\ref{sec-dec}). This, by our humble opinion, can be seen as another contribution of this paper.
%
...
%over approximation to reduce the number of subcases
% the elimination of linear occurrences of variables by under approximation and synchronize the elimination of exponential occurrences of the same variable in different atomic formulas. We also apply an over approximation to reduce the number of subcases and adopt DFS with preprocessing to find possible solutions. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\hide{
Although the general framework of our algorithm is quite akin to Point's,  it contains the following improvements and corrections: 
\begin{itemize}
...
In fact, other than the theoretical difficulties, in practice, the string-number conversion is quite challenging for state-of-the-art solvers. 
...
our implementation manages to solve several variants of the string-hash examples as well as some randomly generated problem instances better than the state-of-the-art solvers. 
